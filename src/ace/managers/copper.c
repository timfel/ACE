...
#ifdef ACE_DEBUG
static const char* _custom_register_names[256] = {
    "BLTDDAT",
    "DMACONR",
    "VPOSR",
    "VHPOSR",
...
void copDumpCmd(tCopCmd *pCmd) {
	if(pCmd->sWait.bfIsWait) {
		const char* cmd = pCmd->sWait.bfIsSkip ? "SKIP" : "WAIT";
		logWrite(
			"@%p: %08lX - %s: %hu,%hu%s\n",
			pCmd, pCmd->ulCode, cmd, pCmd->sWait.bfWaitX << 1, pCmd->sWait.bfWaitY,
			!pCmd->sWait.bfBlitterIgnore ? " & blit done" : ""
		);
	}
	else {
#ifdef ACE_DEBUG
		const char* name = _custom_register_names[pCmd->sMove.bfDestAddr >> 1];
		if (name) {
			logWrite(
				"@%p: %08lX - MOVE: %8s := %04X\n",
				pCmd, pCmd->ulCode, name, pCmd->sMove.bfValue
			);
		}
		else {
			logWrite(
